...
class Song_info(object):
    ...
    def get_song_info(self):
        same_url = "https://music.163.com/#/"
        href = ""
        #url = "https://music.163.com/#/artist?id={0}".format(self.id)
        url="https://music.163.com/#/playlist?id={0}".format(self.id)
        browser.get(url)
        browser.switch_to.frame('contentFrame')
        # 获取歌手的姓名，并建立对应的文件夹
        #artist_name = browser.find_element_by_id("artist-name").text
        artist_name = browser.find_element_by_class_name("s-fc7").text
        print(artist_name)
        path = "f:\\网易云音乐\\{0}".format(artist_name)
        if not os.path.exists(path):
            os.makedirs(path)
        data = browser.find_element_by_class_name("n-songtb").find_elements_by_tag_name('tr')
